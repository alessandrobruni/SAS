%include "/sasdata_MA/workspace/Nexi_Acq_Workspace/dataroot/Ale/_attitude/excltags.tpl";
/**/
...
/*Loads  POS of A4U network*/
/* ID_TP_CANALE_CONV	CO_CANALE	TE_CANALE
	11					CJA			CJ POS agents for you */
/* ID_TP_CANALE_CONV	CO_CANALE	TE_CANALE
	14					25			Agente */

/*
select oll POS of Agent4U network
*/
proc sql;
create table a4u as
select * 
...
/*
Simple display of frequences by POS type , for check pourpose
*/
...
/*
Displeay frequences by channel and year activation, for check pourpose
*/
proc freq data=a4u order=internal ;
...
/* 
Table with some anagrafical POS info 
...
proc sql;
	create table work.start_pos as 
		select distinct  t2.co_soc_piva as Cliente__Partita_IVA
			   ...
			   ,put(month(datepart(t2.dt_ese_att)),MeseItaliano.) as  MESE_ATT_ESE
			   ,year(datepart(t2.dt_ese_ann)) as ANNO_ANN_ESE
			   ,put(month(datepart(t2.dt_ese_ann)),MeseItaliano.) as  MESE_ANN_ESE

    		   ,year(datepart(t1.DT_POS_ATT))  as ANNO_ATT_POS_GT
			   ,put(month(datepart(t1.DT_POS_ATT)),MeseItaliano.) as  MESE_ATT_POS_GT
			   ,year(datepart(t1.DT_POS_ANN)) as ANNO_ANN_POS_GT
			   ,put(month(datepart(t1.DT_POS_ANN)),MeseItaliano.) as  MESE_ANN_POS_GT
...
/*finela part , bring all together*/
...
/* 
send the imel with the attached excel report
*/
